...
if window?
	(() ->
		...
		if (s = ua.match(/(iphone|ipad).*os\s([\d_]+)/))
			dorado.os.ios = parseFloat(s[2]) or -1
			dorado.device.pad = s[1] == "ipad"
			dorado.device.phone = !dorado.device.pad
		else
		...
		dorado.device.mobile = !!(`("ontouchstart" in window)` and ua.match(/(mobile)/))
		dorado.device.desktop = !dorado.device.mobile

		if dorado.device.mobile and !dorado.os.ios
			theshold = 768
			if dorado.browser.qqbrowser
				dorado.device.pad = document.body.clientWidth >= theshold or document.body.clientHeight >= theshold
			else
				dorado.device.pad = window.screen.width >= theshold or window.screen.height >= theshold
			dorado.device.phone = !dorado.device.pad
		return
	)()
