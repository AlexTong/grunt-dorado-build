...
class dorado.Element
	...
	destroy: ()->
		...
		if @elementAttrBinding instanceof dorado.ElementAttrBinding
			@elementAttrBinding.destroy()
		else if @elementAttrBinding instanceof Array
			for elementAttrBinding in @elementAttrBinding
				elementAttrBinding.destroy()
	...
	_set: (attr, value, ignoreError) ->
		if typeof value == "string" and dorado.currentScope
		...
		if @constructor.ATTRIBUTES.hasOwnProperty(attr)
			...
			if @constructor.EVENTS.hasOwnProperty(eventName)
				if value instanceof dorado.Expression
					...
					action = dorado.currentScope?.action(value)
					if action
						if action.isWrapper
							listener = () ->
								return action()
	...
	_doSet: (attr, attrConfig, value) ->
		if value instanceof dorado.Expression and dorado.currentScope
			...
			if !@elementAttrBinding
				@elementAttrBinding = elementAttrBinding
			if @elementAttrBinding instanceof dorado.ElementAttrBinding
				@elementAttrBinding = [@elementAttrBinding, elementAttrBinding]
			else if @elementAttrBinding instanceof Array
				@elementAttrBinding.push(elementAttrBinding)
