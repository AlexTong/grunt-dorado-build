class dorado.AbstractItemGroup extends dorado.Widget
	@ATTRIBUTES:
		items:
			setter: (value)->
				@clearItems()
	...
	getItemDom: (item)->
		...
		if itemConfig instanceof dorado.Widget
	...
	_doOnItemsChange: ()->
		dorado.util.delay(@, "_refreshItems", 50, @refreshItems)
		return

	refreshItems: ()->
		dorado.util.cancelDelay(@, "_refreshItems")
		return @
	...
	addItem: (config)->
		if config.constructor == Object.prototype.constructor
			...
			if config.$type
				item = dorado.widget(config)
			else
				item = $.xCreate(config)

		else if config.nodeType == 1
			active = dorado.util.hasClass(config, "active")
			item = dorado.widget(config)
			item ?= config
	...
	removeItem: (item)->
		...
		if itemObj instanceof dorado.Widget
			itemObj.destroy()
		else
			$(itemObj).remove()
		@_doOnItemsChange()
