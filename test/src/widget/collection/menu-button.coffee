...
class dorado.ButtonMenu extends dorado.Menu
	onItemClick: (event, item)->
		@_parent?.onItemClick(event, item)
		return null

	_toDropDown: (item)->
		unless @_parent instanceof dorado.MenuButton
			super(item)
		return null

class dorado.MenuButton extends dorado.Button
	...
	_parseDom: (dom)->
		...
		while child
			if child.nodeType == 1
				menu = dorado.widget(child)
				if menu and menu instanceof dorado.Menu
					@_menu = menu
					break
