class dorado.Carousel extends dorado.AbstractItemGroup
	@CLASS_NAME: "carousel"
	...
	_parseDom: (dom)->
		...
		while child
			if child.nodeType == 1
				if !@_doms.wrap and dorado.util.hasClass(child, "items-wrap")
					@_doms.wrap = child
					parseItem(child)
				else if !_doms.indicators and dorado.util.hasClass(child, "indicators")
