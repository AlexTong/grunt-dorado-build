...
dorado.shape ?= {}
class dorado.shape.Side extends dorado.Widget
	@ATTRIBUTES:
		content:
			refreshDom: true
			setter: (value)->
	...
	_refreshContent: ()->
		content = @_content
		dom=null
		if content instanceof dorado.Widget
			dom=content.getDom()
		else if content.constructor == Object.prototype.constructor
			if content.$type
				content = @_content = dorado.widget(content)
...
class dorado.Shape extends dorado.Widget
	@CLASS_NAME: "ui shape"
	@ATTRIBUTES:
		sides:
			setter: (sides)->
	...
	addSide: (config)->
		...
		if config instanceof dorado.Widget
			side = new dorado.shape.Side({
				content: config
			})
			...
			if config.$type or config.tagName
				...
				side = new dorado.shape.Side(config)
	...
	removeSide: (side)->
		...
		if side instanceof dorado.shape.Side
