class dorado.Select extends dorado.AbstractInput
	@CLASS_NAME: "input"
	@ATTRIBUTES:
		options:
			setter: (options) ->
				return unless options instanceof Array or options instanceof dorado.EntityList
				@_options = options
				select = @_doms?.input
	...
	_refreshSelectOptions: (select) ->
		...
		if @_options instanceof dorado.EntityList
			options.length = @_options.entityCount
		else
			options.length = @_options.length

		dorado.each @_options, (optionValue, i) ->
			option = options[i]
