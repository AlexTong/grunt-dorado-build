class dorado.RadioButton extends dorado.Widget
	@CLASS_NAME: "checkbox"
	@INPUT_TYPE: "radio"
	@ATTRIBUTES:
		type:
	...
	_refreshEditorDom: ()->
		@get$Dom().checkbox(if @_checked then "check" else "uncheck")
...
class dorado.RadioGroup extends dorado.AbstractEditor
	@CLASS_NAME: "grouped"
	@ATTRIBUTES:
		name: null
		items:
	...
	_parseDom: (dom)->
		...
		while child
			if child.nodeType == 1
				...
				@_addItem(widget) if widget and widget instanceof dorado.RadioButton
			child = child.nextSibling
	...
	_addItem: (item)->
		...
		if item instanceof dorado.RadioButton
			radioBtn = item
		else if item.constructor == Object.prototype.constructor
			radioBtn = new dorado.RadioButton(item)
		else
			classType = dorado.util.getType(item)
			if classType is "number" or classType is "string"
				radioBtn = new dorado.RadioButton({
					value: item
				})
	...
	removeRadioButton: (index)->
		...
		if index instanceof dorado.RadioButton
			radio=index
		else
