...
class dorado.ItemsView extends dorado.Widget
	@ATTRIBUTES:
		allowNoCurrent:
			defaultValue: true
		currentItem:
			getter: () ->
				if @_currentItemDom
					item = dorado.util.userData(@_currentItemDom, "item")
				return item
			setter: (currentItem) ->
				if currentItem
	...
	_onItemsWrapperScroll: () ->
		...
		if @_autoLoadPage and not @_loadingNextPage and (realItems == @_realOriginItems or !@_realOriginItems)
			if realItems instanceof dorado.EntityList and realItems.pageNo < realItems.pageCount
	...
	_createNewItem: (itemType, item) ->
		...
		if template
			...
			itemDom.setAttribute("d-bind", "$default")
	...
	_setCurrentItemDom: (currentItemDom) ->
		if @_currentItemDom
			$fly(@_currentItemDom).removeClass(dorado.constants.COLLECTION_CURRENT_CLASS)
		@_currentItemDom = currentItemDom
		if currentItemDom and @_highlightCurrentItem
			$fly(currentItemDom).addClass(dorado.constants.COLLECTION_CURRENT_CLASS)
		return
	...
	_convertItems: (items) ->
		if @_filterCriteria
			if @getListeners("filterItem")
				...
				items = dorado.convertor.filter(items, (item) =>
					arg.item = item
					return @fire("filterItem", @, arg)
				)
			else
				items = dorado.convertor.filter(items, @_filterCriteria)
		return items
	...
	_doRefreshItems: (itemsWrapper) ->
		...
		if items
			...
			if @_currentItemDom
				if !currentItem
					currentItem = dorado.util.userData(@_currentItemDom, "item")
				$fly(@_currentItemDom).removeClass(dorado.constants.COLLECTION_CURRENT_CLASS)
			...
			if @_currentItemDom and @_highlightCurrentItem
				$fly(@_currentItemDom).addClass(dorado.constants.COLLECTION_CURRENT_CLASS)

			if documentFragment
				itemsWrapper.appendChild(documentFragment)

			if @_autoLoadPage and not @_loadingNextPage and (items == @_realOriginItems || !@_realOriginItems) and items instanceof dorado.EntityList
				...
				if currentPageNo and currentPageNo < items.pageCount
					if itemsWrapper.scrollHeight and (itemsWrapper.scrollTop + itemsWrapper.clientHeight) == itemsWrapper.scrollHeight
						setTimeout(() ->
							items.loadPage(currentPageNo + 1, dorado._EMPTY_FUNC)
							return
						, 0)
		...
		if @_pullAction == undefined
			...
			if hasPullAction
				dorado.util.delay(@, "createPullAction", 200, @_createPullAction)
		return
	...
	_refreshItemDom: (itemDom, item, parentScope = @_itemsScope) ->
		...
		itemScope = dorado.util.userData(itemDom, "scope")
		oldScope = dorado.currentScope
		try
			if !itemScope
				itemScope = new dorado.ItemScope(parentScope, alias)
				dorado.currentScope = itemScope
				itemScope.data.setTargetData(item, true)
				dorado.util.userData(itemDom, "scope", itemScope)
				dorado.util.userData(itemDom, "item", originItem)
				@_doRefreshItemDom?(itemDom, item, itemScope)
				dorado.xRender(itemDom, itemScope, @_templateContext)
			else
				dorado.currentScope = itemScope
				if itemScope.data.getTargetData() != item
					delete @_itemDomMap[itemDom._itemId] if itemDom._itemId
					if itemScope.data.alias != alias
						throw new dorado.Exception("Repeat alias mismatch. Expect \"#{itemScope.alias}\" but \"#{alias}\".")
					dorado.util.userData(itemDom, "item", originItem)
	...
	_onItemClick: (evt) ->
		...
		item = dorado.util.userData(itemDom, "item")
		if itemDom._itemType == "default"
			if item
				if @_changeCurrentitem and item._parent instanceof dorado.EntityList
	...
	_bindEvent: (eventName) ->
		if eventName == "itemPress"
			@_on("press", (self, arg) =>
				...
				if itemDom
					arg.itemDom = itemDom
					arg.item = dorado.util.userData(itemDom, "item")
	...
	_createPullAction: () ->
		@_pullAction = new dorado.PullAction(@_doms.itemsWrapper, {
...
dorado.Element.mixin(dorado.ItemsView, dorado.TemplateSupport)
dorado.Element.mixin(dorado.ItemsView, dorado.DataItemsWidgetMixin)
