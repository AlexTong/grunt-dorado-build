class TreeNode extends dorado.Node
	@ATTRIBUTES:
		expanded:
			getter: () ->
...
class TreeNodeBind extends dorado.CascadeBind
	@NODE_TYPE: TreeNode
...
class dorado.Tree extends dorado.ItemsView
	...
	_refreshItemDom: (itemDom, node, parentScope) ->
		...
		if !itemDom._binded
			...
			if itemDom._itemType == "checkable"
				...
				if checkboxDom
					tree = @
					checkbox = dorado.widget(checkboxDom)
					checkbox.set(
	...
	_onItemClick: (evt) ->
		...
		node = dorado.util.userData(itemDom, "item")
		@_setCurrentNode(node)
		return super(evt)
	...
	expand: (node) ->
		...
		if !itemsScope
			node._itemsScope = itemsScope = new dorado.ItemsScope(node._scope)
...
dorado.Element.mixin(dorado.Tree, dorado.TreeSupportMixin)