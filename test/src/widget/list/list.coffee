SAFE_SLIDE_EFFECT = dorado.os.android and !dorado.browser.chrome
...
class dorado.ListView extends dorado.ItemsView
	...
	_initDom: (dom) ->
		...
		dorado.util.delay(@, "initItemSlide", 200, @_initItemSlide)
		return
	...
	_refreshItems: () ->
		...
		if @_dom
			...
			if !dorado.os.mobile and !@_indexBarRelocateTimer
				itemsWrapper = @_doms.itemsWrapper
				dom = @_dom
	...
	_refreshGroupDom: (groupDom, group, parentScope = @_itemsScope) ->
		groupId = dorado.Entity._getEntityId(group)

		groupScope = dorado.util.userData(groupDom, "scope")
		if !groupScope
			groupDom._itemScope = groupScope = new dorado.ItemScope(parentScope, group._alias)
			parentScope.regItemScope(groupId, groupScope)
			groupScope.data.setTargetData(group, true)
			dorado.util.userData(groupDom, "scope", groupScope)
			dorado.util.userData(groupDom, "item", group)
		else
			oldGroup = dorado.util.userData(groupDom, "item")
		...
		if !groupDom._headerCreated
			...
			dorado.xRender(groupHeaderDom, groupScope, @_templateContext)
			groupDom.insertBefore(groupHeaderDom, itemsWrapper)
			dorado.util.userData(groupHeaderDom, "item", group)
		else
	...
	_onItemsWrapperScroll: () ->
		...
		if topGroupDom
			...
			group = dorado.util.userData(topGroupDom, "item")
	...
	_getFloatGroupHeader: (group) ->
		...
		if !floatGroupHeaderWrapper
			groupScope = new dorado.ItemScope(@_itemsScope, group._alias)
			groupScope.data.setTargetData(group, true)
			floatGroupHeader = @_createNewItem("group-header", group)
			dorado.util.userData(floatGroupHeader, "scope", groupScope)
			@_templateContext.defaultPath = group._alias
			dorado.xRender(floatGroupHeader, groupScope)
			...
			groupScope = dorado.util.userData(floatGroupHeader, "scope")
			groupScope.data.setTargetData(group)
	...
	_initItemSlide: () ->
		...
		itemScope = new dorado.ItemScope(@_itemsScope, @_alias)
		...
		if @_leftItemSlide and leftSlidePaneTemplate
			...
			dorado.xRender(leftSlidePaneTemplate, itemScope, @_templateContext)
			dorado.util.userData(leftSlidePaneTemplate, "scope", itemScope)
		...
		if @_rightItemSlide and rightSlidePaneTemplate
			...
			dorado.xRender(rightSlidePaneTemplate, itemScope, @_templateContext)
			dorado.util.userData(rightSlidePaneTemplate, "scope", itemScope)
			dorado._ignoreNodeRemoved = true
	...
	_onItemsWrapperTouchMove: (evt) ->
		...
		if direction != @_itemSlideDirection
			...
			if slidePane
				itemScope = dorado.util.userData(slidePane, "scope")
