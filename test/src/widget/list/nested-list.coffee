class NestedListNode extends dorado.Node
	@ATTRIBUTES:
		title:
			readOnly: true
			getter: () ->
...
class NestedListBind extends dorado.CascadeBind
	@NODE_TYPE = NestedListNode
	@ATTRIBUTES:
		titleProperty: null

class dorado.NestedList extends dorado.Widget
	...
	_createLayer: (index) ->
		...
		if @_showTitleBar
			...
			hjson =
				...
				"d-widget": if useLayer then "layer" else "widget"
				content:
					tagName: "div"
					class: "v-box"
					style:
		...
		ctx = {}
		new dorado.xRender(hjson, @_scope, ctx)

		list = dorado.widget(ctx.list)
		...
		if ctx.container
			container = dorado.widget(ctx.container)
		else
			container = list
		...
		layer = {
			...
			titleBar: dorado.widget(ctx.titleBar)
			list: list
			container: container
		}
		return layer
	...
	_showLayer: (index, parentNode, callback) ->
		...
		parentNode._bind.retrieveChildNodes(parentNode, () ->
			if parentNode._children
				...
				layer.container.show() if layer.container instanceof dorado.Layer
	...
	_hideLayer: (animation) ->
		...
		if layer.container instanceof dorado.Layer
...
dorado.Element.mixin(dorado.NestedList, dorado.TemplateSupport)