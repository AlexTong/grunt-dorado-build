dorado.slotPicker ?= {}
class dorado.slotPicker.ZyngaScroller extends dorado.Element
	@EVENTS:
		scrolled: null
	constructor: (container, options)->
		...
		options.scrollingComplete = ()->
			dorado.util.delay(self, "scrolled", 50, self._scrolled)
	...
	scrollSize: (dir, container, content)->
		translate = dorado.Fx.getElementTranslate(content)
		dorado.Fx.cancelTranslateElement(content)
		...
		dorado.Fx.translateElement(content, translate.left, translate.top)
		return result
	...
	destroy: ()->
		dorado.util.cancelDelay(@, "scrolled")
		if dorado.os.mobile
...
class dorado.AbstractSlotList extends dorado.RenderableElement
	_resetDimension: ()->
		return
	_setDom: (dom)->
...
class dorado.SlotList extends dorado.AbstractSlotList
	@CLASS_NAME: "list"
	@ATTRIBUTES:
		viewItemCount:
			refreshDom: true
...
#			refreshDom: true
	...
	doTouchStart: (touches, timeStamp)->
		dorado.slotPicker._activePicker = @
	...
	doTouchEnd: (timeStamp)->
		...
		dorado.slotPicker._activePicker = null
		return
	...
	_setDom: (dom)->
		...
		list._zyngaScroller = new dorado.slotPicker.ZyngaScroller(dom, {
			render: dorado.util.getScrollerRender(@_doms.body)
		})
...
class dorado.RangeSlotList extends dorado.SlotList
	@ATTRIBUTES:
		range:
			refreshDom: true
			setter: (value)->
...
class dorado.MultiSlotPicker extends dorado.AbstractSlotList
...
#		dorado.util.delay(picker, "$refreshDelayTimerId", 50, picker.refresh)
...
class dorado.mobile.DateTimePicker extends dorado.MultiSlotPicker
	@CLASS_NAME: "multi-slot-picker"
	@slotConfigs: []
	@ATTRIBUTES:
		type:
...
dorado.mobile.showDateTimePicker = (options)->
	timerLayer = dorado.mobile._cacheDateTimer
	if timerLayer
		if options.type isnt timerLayer._picker.get("type")
			timerLayer.destroy()
			dorado.mobile._cacheDateTimer = null
			timerLayer = null

	unless timerLayer
		picker = new dorado.mobile.DateTimePicker({type: options.type || "date"})
		timerLayer = new dorado.Layer({
...
#		timerLayer.set("content",picker)
		...
		dorado.mobile._cacheDateTimer = timerLayer
	timerLayer = dorado.mobile._cacheDateTimer
	options ?= {}
	if options.onHide
