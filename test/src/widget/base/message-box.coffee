#dorado.os.mobile = true
do()->
	dorado.commonDimmer =
		show: ()->
			_dimmerDom = dorado.commonDimmer._dom
			unless _dimmerDom
				_dimmerDom = $.xCreate({
					tagName: "Div"
				...
				dorado.commonDimmer._dom = _dimmerDom
			$(_dimmerDom).addClass("active")
			return

		hide: ()->
			$(dorado.commonDimmer._dom).removeClass("active")

	messageBox =
		animation: if dorado.device.phone then "slide up" else "scale"
		settings:
			info:
				title: "消息"
				icon: "blue info icon"
	...
	createMessageBoxDom = ()->
		...
		dom = $.xCreate(
			{
				tagName: "Div"
				class: "ui #{if dorado.device.phone then "mobile layer" else "desktop"} message-box transition hidden"
		...
		actionsDom = $.xCreate({
			tagName: "div"
			class: "actions #{if dorado.device.phone then "ui buttons two fluid top attached" else ""}"
			contextKey: "actions"
			content: [
				{
					tagName: "div"
		...
		if dorado.device.phone then $(doms.content).before(actionsDom) else doms.contentContainer.appendChild(actionsDom)
	...
	messageBox.getDom() if dorado.os.mobile
	dorado.MessageBox=messageBox