do->
	...
	class dorado.calendar.DateGrid extends dorado.RenderableElement
		@ATTRIBUTES:
			calendar: null
			columnCount:
				defaultValue: 1
		...
		_createDom: ()->
			...
			$fly(dom).on("click", (event)->
				...
				if position and position.element
					...
					picker.fire("onCellTab", picker, position)
			)

			return dom

		doFireRefreshEvent: (eventArg) ->
			@fire("onCellRefresh", @, eventArg)
			return @
		...
		doFireRefreshEvent: (eventArg)->
			...
			if @_state && @_year && @_month
				...
				eventArg.date = new Date(year, month, cellState.text)
	...
	class dorado.calendar.SwipePicker extends dorado.RenderableElement
		...
		createDateTable: (dom)->
			cal = @_calendar
			dateTable = new dorado.calendar.DateGrid({
				rowCount: 6
				columnCount: 7
				calendar: cal
				tableClassName: "date-table"
				onCellRefresh: (self, arg)->
					cal.fire("onCellRefresh", cal, arg)
				onCellTap: (self, arg)->
					element = arg.element
					state = self._state
					...
					if cellState.type == "prev-month"
						picker.prev(() ->
							cal.prevMonth()
							cal.setDate(cellState.text)
							cal.fire("onDateChange", cal, {})
						)
					else if cellState.type == "next-month"
						picker.next(()->
							cal.nextMonth()
	...
	class dorado.Calendar extends dorado.Widget
		@CLASS_NAME: "calendar"
		@ATTRIBUTES:
			date:
				getter: ()->
		...
		_createDom: ()->
			...
			picker = cal._datePicker = new dorado.calendar.SwipePicker({
